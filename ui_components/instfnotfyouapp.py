...
class InstFnotFYouApp(ft.Column):
    def __init__(self, *args, **kwargs):
        ...
        self.add_file_button = ft.TextButton(
            icon=ft.icons.FILE_OPEN, text="Add zip file", on_click=self.get_file_name
        )
        self.submit_button = ft.IconButton(
            icon=ft.icons.CHECK_SHARP, on_click=self.controls_adder
        )
    ...
    def searcher(self, text):
        self.list_view.controls[0].rows = [
            ft.DataRow([ft.DataCell(control[0]), ft.DataCell(control[1])])
            for control in self.info_ctrls
            if text in control[0].value
        ]
        self.update()
    ...
    def show_usernames(self):
        ...
        search_bar = SearchBar(
            func=self.searcher, border_radius=10, width=self.page.width / 2
        )
            ...
            [
                ft.DataTable(
                    columns=[
                        ...
                        ft.DataColumn(ft.Text("Follow Date")),
                    ],
                    rows=[
                        ft.DataRow([ft.DataCell(control[0]), ft.DataCell(control[1])])
                        for control in self.info_ctrls
        ...
        self.list_view.scroll_to(
            delta=0.0000001
        )  # making it so small that it's negligible (fizics 🤡)
        self.update()
